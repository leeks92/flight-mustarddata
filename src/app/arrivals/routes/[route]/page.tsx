...
import { getArrivalRoutes, getArrivalRoute, getAirport, getMetadata, formatSeason } from '@/lib/data';
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const seoMeta = stats
    ? generateRouteSeoMeta(parsed.depCode, parsed.arrCode, depName, arrName, stats, 'arrival')
    : null;

  const title = seoMeta?.title || `${depName} → ${arrName} 도착편 정기운항 시간표`;
  const description = seoMeta?.description || `${depName}에서 ${arrName}까지 정기운항 도착편 시간표입니다.`;
...
export default async function ArrivalRoutePage({ params }: Props) {
  ...
  const seoMeta = generateRouteSeoMeta(parsed.depCode, parsed.arrCode, depName, arrName, stats, 'arrival');
  const descriptions = generateRouteDescription(parsed.depCode, parsed.arrCode, depName, arrName, stats, 'arrival', seasonLabel);
  const faqItems = generateRouteFAQ(parsed.depCode, parsed.arrCode, depName, arrName, stats, 'arrival');
  const relatedLinks = getRelatedRoutes(parsed.depCode, parsed.arrCode, 'arrival');
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8">
      ...
      <FlightTable flights={sortedFlights} type="arrival" />
